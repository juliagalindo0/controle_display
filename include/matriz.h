...
#include "controle_display.pio.h"
...
void matrizInit(uint pin, npLED_t leds[]) {
  ...
  uint offset = pio_add_program(pio0, &controle_display_program);
  np_pio = pio0;
  ...
  controle_display_program_init(np_pio, sm, offset, pin, 800000.f, false); // ou true, dependendo do seu LED
